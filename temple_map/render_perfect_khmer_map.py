...
# -*- coding: utf-8 -*-
"""
Perfect Khmer Temple Map Annotator
Fixes descender clipping (ជើងអក្សរ) with extra vertical margin and descent padding.
"""
...
ORIG_MARKERS = [
    ...
    ("ចេតីយឧកញ៉ា ចាយ ស៊ាងអ៊ី",              514, 759, "building"),
...
def draw_clean_label(draw, img_w, img_h, name, tx, ty, category, font):
    ...
    # Generous padding to prevent clipping Khmer descenders (ជើងអក្សរ)
    px = 10
    py_top = 4
    py_bottom = 10  # Extra space for Khmer descenders

    box_w = nw + px * 2
    box_h = nh + py_top + py_bottom
    ...
    # Box background with elegant rounded corners
    draw.rounded_rectangle([bx1, by1, bx2, by2], radius=6, fill=(15, 23, 42, 235), outline=border_color, width=2)

    # Text positioning with proper offset for top padding and descender breathing room
    text_x = bx1 + px - nm_bb[0]
    text_y = by1 + py_top - nm_bb[1]

    # Black text outline for maximum legibility
...
def main():
    ...
    W, H = img.size
    print(f"Annotating clean image with full Khmer descenders ({W}x{H})...")
    ...
    print("Saved temple_map_labeled.jpg with NO descender clipping!")
